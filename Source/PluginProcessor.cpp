...
void ThunderforgeAudioProcessor::loadPresets()
{
    ...
    {
        // Write default AC/DC presets
        juce::Array<juce::var> array;
        auto addPreset = [&](const juce::String& name, double d, double b, double m, double t, double p, double v) {
        ...
        addPreset("BACK IN BLACK", 7.5, 5.0, 8.5, 5.0, 6.0, -4.0);
        addPreset("HIGHWAY TO HELL", 6.5, 6.0, 7.0, 6.0, 5.0, -4.0);
        addPreset("THUNDERSTRUCK", 8.5, 8.0, 6.0, 8.0, 7.0, -2.0);
        addPreset("HELLS BELLS", 4.5, 5.0, 8.0, 3.0, 2.0, -1.0);
        addPreset("YOU SHOOK ME", 6.0, 4.0, 7.5, 7.0, 6.0, -3.0);

        juce::var defaultPresets(array);
        juce::String jsonString = juce::JSON::toString(defaultPresets);
    ...
    if (parsedJson.isArray())
    {
        ...
        {
            auto& obj = parsedJson[i];
            thunderforge::Preset p;
            p.name = obj["name"].toString();
            p.drive = static_cast<float>(static_cast<double>(obj["drive"]));
            p.bass = static_cast<float>(static_cast<double>(obj["bass"]));
            p.mid = static_cast<float>(static_cast<double>(obj["mid"]));
            p.treble = static_cast<float>(static_cast<double>(obj["treble"]));
            p.presence = static_cast<float>(static_cast<double>(obj["presence"]));
            p.volume = static_cast<float>(static_cast<double>(obj["volume"]));
            presets.push_back(p);
        }
    }
}
...
void ThunderforgeAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    for (const auto metadata : midiMessages)
    {
        ...
        if (message.isController())
        {
            ...
            float val = message.getControllerValue() / 127.0f; // Normalize 0.0 - 1.0

            if (cc >= 1 && cc <= 10)
            {
                static constexpr std::array<const char*, 10> paramIDs = {
                    "gate_threshold", "comp_threshold", "ts_drive", "eq_bass",
                    "eq_mid", "eq_treble", "delay_mix", "reverb_mix",
                    "output_gain", "stereo_width"
                };

                auto param = apvts.getParameter (paramIDs[cc - 1]);
                if (param)
                    param->setValue (val);
            }
        }
...
void ThunderforgeAudioProcessor::loadPreset (int index)
{
    if (index >= 0 && index < presets.size())
    {
        ...
        setParam ("ts_drive", presets[index].drive * 10.0f, 0.0f, 100.0f);
