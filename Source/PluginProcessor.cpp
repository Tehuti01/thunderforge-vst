...
{
    int dataSize = 0;
    const char* data = BinaryData::presets_json;
    dataSize = BinaryData::presets_jsonSize;

    if (data != nullptr && dataSize > 0)
    {
        juce::String jsonString (data, (size_t) dataSize);
        juce::var parsedJson;
        juce::Result result = juce::JSON::parse (jsonString, parsedJson);

        if (result.wasOk() && parsedJson.isArray())
        {
            ...
            for (auto& item : *array)
            {
                if (item.isObject())
                {
                    ...
                    p.shortName = item["shortName"].toString();
...
void ThunderforgeAudioProcessor::loadPreset (int index)
{
    if (index >= 0 && index < presets.size())
    {
        ...
        setParam ("master_volume", p.volume, -60.0f, 12.0f);
...
{
    if (index >= 0 && index < presets.size()) return presets[index].shortName;
    return "USER";
}
