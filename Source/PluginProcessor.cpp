      ...
      window (fftSize, juce::dsp::WindowingFunction<float>::hann)
{
    ...
    juce::File presetFile = getPresetsFile();
    ...
    if (! presetFile.existsAsFile())
    {
        ...
        savePresetsToJSON (presetFile);
    }
    else
    {
        loadPresetsFromJSON (presetFile);
    }
}

juce::File ThunderforgeAudioProcessor::getPresetsFile() const
{
...
void ThunderforgeAudioProcessor::loadPresetsFromJSON (const juce::File& file)
{
    if (! file.existsAsFile()) return;

    juce::var jsonVar = juce::JSON::parse (file);
    if (! jsonVar.isArray()) return;

    presets.clear();
    auto* array = jsonVar.getArray();
    for (auto& var : *array)
    {
...
void ThunderforgeAudioProcessor::savePresetsToJSON (const juce::File& file)
{
    ...
    juce::var arrayVar (array);
    juce::String jsonString = juce::JSON::toString (arrayVar);
